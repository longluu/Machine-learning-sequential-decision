function Experiment_DriverPTB_MissedTrials(params, fileLoad)
...
try
    ...
    trialIndexMissed = find(isnan(dataTotal(:,7))); 
    nTrialMissed = length(trialIndexMissed);
    loopIndex = 1;
    while loopIndex <= nTrialMissed
        trialIndex = trialIndexMissed(loopIndex);
        repeatFlag = 0;

        % Dertermine which condition to use
        decisionGiven = (trialIndex <= nCategoryGivenTrial);

        % boundary line  
        angleReference = dataTotal(trialIndex,5);

        % Pick the angle difference
        if ~params.staircase
            angleDiff = dataTotal(trialIndex,1);
        else
        ...
        if decisionGiven
            ...
            if params.useModel
                ...
                measureSample = angleDiff + randn * sigmaModel(params.barAspectRatio == dataTotal(trialIndex,4));
                if measureSample > pseModel(params.barAspectRatio == dataTotal(trialIndex,4))
                    decisionCW = 1;
                elseif measureSample < pseModel(params.barAspectRatio == dataTotal(trialIndex,4))
                    decisionCW = -1;
                else
                    if rand > 0.5
                        decisionCW = 1;
        ...
        % Present bars
        angleBarStd = dataTotal(trialIndex,4);
        ...
        while (mglGetSecs - startTimebar) < dataTotal(trialIndex,2)
        end
        if params.instruction
            pause
        end 
        ...
        if repeatFlag
            temp = trialIndex;
            trialIndex = trialOrder(end);
            trialOrder(end) = temp;
            params.trialOrder = trialOrder;
        else
            % Get the response for estimation task
            if (trialIndex <= nCategoryGivenTrial) ...
                    || ((trialIndex >= nCategoryGivenTrial) ...
                            && (trialIndex <= nCategoryGivenTrial + nEstimationTrial))
                mglClearScreen(0.5);
                mglBltTexture(boundaryTexture,[0 0],0,0,angleReference)
                mglPolygon(xVertex, yVertex, params.fixation1(5:7)) 
                ...
                if ~decisionGiven
                    sound(yHigh,fs)
                elseif dataTotal(trialIndex,6) == 1
                    sound(yHigh,fs)
                elseif dataTotal(trialIndex,6) == 0
                    sound(yLow,fs)
                end
                        ...
                        case 1
                            if ~isempty(thetaLine)
                                ...
                                if (thetaEstimate > 180) || (thetaEstimate < 0) 
                                    dataTotal(trialIndex,7) = thetaEstimate-sign(thetaEstimate)*180;
                                else
                                    dataTotal(trialIndex,7) = thetaEstimate;
                                end                                
                                keepLooping = false;  
                            end
                    end
