function [fitParameter, negLogLH] = modelFitBayesJoint_New(optimizationAlgorithm, SetStartPoint, initialValue, fileID, expNumber, plotFitProgress, modelType, dataName, fixMotorNoise, includeIncongruentTrials, fixLapseRate)
...
b_Inequality = [0; 0];
vlb = [minStd minStd minStd  0            10  minStdMemory 1e-2 minStdMotor];
vub = [maxStd maxStd maxStd  maxLapse     42  maxStdMemory 1   maxStdMotor];
if SetStartPoint
...
optimFun = @(searchParameter, varargin) computeNegLLH(searchParameter(1:3),searchParameter(4),...
    ...
    nTrialsPerCondition, estimateDataExp2, estimateDataExp3, fileID, expNumber, plotFitProgress, modelType, includeIncongruentTrials);
...
function errorTotal = computeNegLLH(stdSensory, lapseRate, priorRange, stdMemory, smoothFactor, stdMotor, angleDiff,...
                            percentCW, nTrials, estimateDataExp2, estimateDataExp3, fileID, expNumber, plotFitProgress, modelType, includeIncongruentTrials)
global hOptim
global negLLH

try
    %% Compute the full model prediction for both Exp 2
    pC = [0.5 0.5]';
    [predictedFractionCW, estimateModelExp2, estimateModelExp3] = fullBayesianJoint(stdSensory, stdMemory, priorRange,...
    ...
    errorTotal1 = -logLikelihoodDiscriminate - logLikelihoodEstimate1;
    
    %% Compute the full model prediction for both tasks in Exp 3
    % Error term for estimation task    
    logLikelihoodEstimate2 = 0;
    tempEstimateModelX = estimateModelExp3.Xval{1};
    for ii = 1 : length(estimateModelExp3.Yval)
        estimateModelY = estimateModelExp3.Yval{ii};
        estimateModelY = estimateModelY(:, angleDiff >=0);
        for jj = 1 : size(estimateModelY,2)            
            tempEstimateModelY = estimateModelY(:, jj); 
            tempEstimateModelY = tempEstimateModelY ./ trapz(tempEstimateModelX, tempEstimateModelY);
            tempestimateDataX = estimateDataExp3{ii,jj};
            tempestimateDataX(tempestimateDataX<min(tempEstimateModelX) | tempestimateDataX>max(tempEstimateModelX)) = [];                
            tempLikelihood = interp1(tempEstimateModelX, tempEstimateModelY, tempestimateDataX, 'pchip');
            tempLikelihood(isnan(tempLikelihood)) = [];
            tempLikelihood(tempLikelihood == 0) = epsZero;            
    ...
    errorTotal2 = -logLikelihoodEstimate2;
    errorTotal = errorTotal1 + errorTotal2;
    
    
    disp(['-logLH:' num2str(round(errorTotal)) ' ' num2str(round(-logLikelihoodDiscriminate)) ...
            ' ' num2str(round(-logLikelihoodEstimate1-logLikelihoodEstimate2)) ', Params: ' num2str([roundn(stdSensory, -2) roundn(lapseRate, -4) roundn([priorRange stdMemory smoothFactor stdMotor],-2)])])
    fprintf(fileID,'%2s %9.1f %8.1f %9.1f %9.1f %9.4f %9.4f %9.4f %16.4f  %10.4f %10.4f %8.4f %9.4f \r\n', '//', errorTotal, -logLikelihoodDiscriminate,...
        -logLikelihoodEstimate1, -logLikelihoodEstimate2, stdSensory(1), stdSensory(2), stdSensory(3), lapseRate, priorRange, stdMemory, smoothFactor, stdMotor);
...
function [pCGthetaAll, estimateModelExp2, estimateModelExp3] = fullBayesianJoint(stdSensory, stdMemory, priorRange, smoothFactor, pC, expNumber, thetaStim, stdMotor, modelType, lapseRate, includeIncongruentTrials)
try
    estimateModelExp2.Xval = cell(1, length(stdSensory));
    estimateModelExp2.Yval = cell(1, length(stdSensory)); 
    estimateModelExp3.Xval = cell(1, length(stdSensory));
    estimateModelExp3.Yval = cell(1, length(stdSensory)); 
    ...
    pthGC_Exp3(1,:) = TukeyWindow([0 priorRange], 0, smoothFactor, th);
    pthGC_Exp3(2,:) = TukeyWindow([-priorRange 0], 1, smoothFactor, th);
    ...
    for kk=1:length(stdSensory)  
        ...
        estimateModelExp2.Xval{kk} = th;
        estimateModelExp2.Yval{kk} = pthhGth;
        
        %% Compute predicted distribution for Exp 3        
        % 2: estimation
        pmmGth = exp(-((MM_th-THmm).^2)./(2*(stdSensory(kk)^2 + stdMemory^2))); % p(mm|th) = N(th, sm^2 + smm^2)
        pmmGth = pmmGth./(repmat(sum(pmmGth,1),nmm,1)); 

        pthGmmChcw = (pmmGth.*repmat(pthGC_Exp3(1,:),nmm,1))';
        pthGmmChcw = pthGmmChcw./repmat(sum(pthGmmChcw,1),nth,1);
        pthGmmChcw(isnan(pthGmmChcw)) = 0;

        pthGmmChccw = (pmmGth.*repmat(pthGC_Exp3(2,:),nmm,1))';
        pthGmmChccw = pthGmmChccw./repmat(sum(pthGmmChccw,1),nth,1);
        pthGmmChccw(isnan(pthGmmChccw)) = 0;            
        ...
        % set the ideal psychometric curve
        ...
        estimateModelExp3.Xval{kk} = th;
        estimateModelExp3.Yval{kk} = pthhGth;        
    end       
catch e
    keyboard
    rethrow(e)
