function [fitParameter, negLogLH] = modelFitBayesJoint123(optimizationAlgorithm, SetStartPoint, initialValue, fileID, expNumber, plotFitProgress, modelType, dataName, fixMotorNoise, includeIncongruentTrials, fixLapseRate)
...
optimFun = @(searchParameter, varargin) computeNegLLH(searchParameter(1:3),searchParameter(4),...
    ...
    nTrialsPerCondition1, percentCW2, nTrialsPerCondition2, estimateDataExp1, estimateDataExp2, estimateDataExp3, fileID, expNumber, plotFitProgress, modelType, includeIncongruentTrials);
...
function errorTotal = computeNegLLH(stdSensory, lapseRate, priorRange1, priorRange2, stdMemory, smoothFactor, stdMotor, angleDiff,...
                            percentCW1, nTrials1, percentCW2, nTrials2, estimateDataExp1, estimateDataExp2, estimateDataExp3, fileID, expNumber, plotFitProgress, modelType, includeIncongruentTrials)
global hOptim
global negLLH
...
try
    ...
    pC = [0.5 0.5]';
    [predictedFractionCW2, estimateModelExp2, estimateModelExp3] = fullBayesianJoint(stdSensory, stdMemory, priorRange2,...
            smoothFactor, pC, angleDiff, stdMotor, modelType, lapseRate, includeIncongruentTrials);    
    ...
    % Error term for estimation task Exp 3
    logLikelihoodEstimate3 = 0;
    ...
    for ii = 1 : length(estimateModelExp3.Yval)
        ...
        for jj = 1 : size(estimateModelY,2)            
            ...
            tempLikelihood(isnan(tempLikelihood)) = [];
            tempLikelihood(tempLikelihood == 0) = epsZero;            
            logLikelihoodEstimate3 = logLikelihoodEstimate3 + nansum(log(tempLikelihood)); 
            if logLikelihoodEstimate3>0
                keyboard
            end                
        end
    end
    errorTotal3 = -logLikelihoodEstimate3;
    errorTotal = errorTotal1 + errorTotal2 + errorTotal3;
    
    
    disp(['-logLH:' num2str(round(errorTotal)) ' ' num2str(round(-logLikelihoodDiscriminate1-logLikelihoodDiscriminate2)) ...
            ' ' num2str(round(-logLikelihoodEstimate1-logLikelihoodEstimate2-logLikelihoodEstimate3)) ', Params: ' num2str([roundn(stdSensory, -2) roundn(lapseRate, -4) roundn([priorRange1 priorRange2 stdMemory smoothFactor stdMotor],-2)])])
    fprintf(fileID,'%2s %9.1f %8.1f %9.1f %9.4f %9.4f %9.4f %16.4f  %10.4f %10.4f %10.4f %8.4f %9.4f \r\n', '//', errorTotal, -logLikelihoodDiscriminate1-logLikelihoodDiscriminate2,...
        -logLikelihoodEstimate1-logLikelihoodEstimate2-logLikelihoodEstimate3,...
...
function [pCGthetaAll, estimateModelExp2, estimateModelExp3] = fullBayesianJoint(stdSensory, stdMemory, priorRange, smoothFactor, pC, thetaStim, stdMotor, modelType, lapseRate, includeIncongruentTrials)
try
