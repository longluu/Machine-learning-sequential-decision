...
dstep = 0.1;
paramsAll = [23.1272   14.7546   11.8721    9.7087           0.0000     20.5409     0.0872   0.9978    6.4726 12.6276];
lapseRate = paramsAll(5);
stdBoundary = paramsAll(10);
...
% the boundary 
nml = 101;
range_nml = 6*stdBoundary;
dstep_nml = round(range_nml/nml, 1);
lowerBound = round(-3*stdBoundary, -log10(dstep));
ml = lowerBound:dstep_nml:lowerBound+(nml-1)*dstep_nml;
pMl = normpdf(ml, 0, stdBoundary);
pMl = pMl / sum(pMl);
...
for kk=1:length(stdSensory)  
    pthhAndth = zeros(nth, length(thetaStim));   
    pthhGthChcwFinal = zeros(nth, length(thetaStim));  
    pthhGthChccwFinal = zeros(nth, length(thetaStim));
    ...
    for ll = 1 : nml
        ...
        pthhGthChcw = pthhGthChcw./repmat(sum(pthhGthChcw,1),nth,1); % normalize - conv2 is not    
        pthhGthChccw = pthhGthChccw./repmat(sum(pthhGthChccw,1),nth,1);  
        pthhGthChcwFinal = pthhGthChcwFinal + pMl(ll) * pthhGthChcw;
        pthhGthChccwFinal = pthhGthChccwFinal + pMl(ll) * pthhGthChccw;

        pthhGth = pthhGthChcw.*repmat(PChGtheta_lapse_new(1,:),nth,1) + pthhGthChccw.*repmat(PChGtheta_lapse_new(2,:),nth,1);
        pthhAndth = pthhAndth + pMl(ll) * pthhGth .* repmat(pthGIndexcongruent, nth, 1);
        disp(['Iteration: ' num2str(ll)])
    end
    mthhGthChcw = th * pthhGthChcwFinal;
    mthhGthChccw = th * pthhGthChccwFinal;
