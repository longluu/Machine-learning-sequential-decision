function [binaryDecision, percentCW, nTrialsPerCondition, angleDiffEstimate, angleDiff, stdMotor] = dataForBootstrap(subjectID, includeIncongruentTrials, correctIndex)
...
for ii = 1 : length(experimentNumber)
    ...
    angleDiffMotor = (unique(dataTotal(:,1)))';
...
%     angleEstimate(params.trialOrder(1:round(nEstimate/2))) = NaN;
    ...
    stdRobust = NaN(1, length(angleDiffMotor));
    for jj = 1 : length(angleDiffMotor)
        indexSelect = dataTotal(:,1)==angleDiffMotor(jj);
