...
tic
subjectIDAll = {'ll', 'pw', 'eh', 'bh', 'ln', 'at', 'dh', 'average'}; % 
logLH_AllModel = NaN(length(subjectIDAll), 3, maxIter+1);
...
logLH_DataRep = repmat(logLH_Data, 1, 1, 3);
logLH_DataRep = permute(logLH_DataRep, [1 3 2]);
lowerBound = repmat(logLH_AllModel(:, 1, :), 1, 3, 1);
logLH_ModelNorm = (logLH_AllModel - lowerBound) ./ (logLH_DataRep - lowerBound);

meanLogLH = squeeze(logLH_ModelNorm(:, 2:3, end));
lowerConfLogLH = NaN(length(subjectIDAll), 2);
upperConfLogLH = NaN(length(subjectIDAll), 2);
