...
% Extract all bootstrapped parameters
...
meanLogLH = squeeze(logLH_ModelNorm(:, 2:size(logLH_AllModel, 2), end));
...
colorName = {'Teal', 'Purple', 'Red', 'DodgerBlue'};
colorIndex = NaN(length(colorName), 3);
for ii = 1 : length(colorName)
    colorIndex(ii, :) = rgb(colorName{ii});
end
