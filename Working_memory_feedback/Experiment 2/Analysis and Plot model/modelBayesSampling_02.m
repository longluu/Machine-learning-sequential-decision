...
flagSC = 1; % 1: self-conditioned model
            % 0: standard Bayes
incorrectType = 1; % 1: flip the decision bit, condition on self decision
                   % 2: flip the decision bit, condition on feedback
                   % 3: flip the estimates
            
params = [5.3271    9.8390           0.0000     45.8895     0.1   0.9968    3.3234];
...
if incorrectType == 1
    for jj = 1 : length(stdSensory)
        rangeM = [min(thetaStim)-5*stdSensory(jj) max(thetaStim)+5*stdSensory(jj)];
        ...
        PChGm = zeros(2, length(m));
        PChGm(1, m>0) = 1;
        PChGm(1, m==0) = 0.5;
        PChGm(2, m<0) = 1;
        PChGm(2, m==0) = 0.5;
        
        pmGth = exp(-((M-THm).^2)./(2*stdSensory(jj)^2));
