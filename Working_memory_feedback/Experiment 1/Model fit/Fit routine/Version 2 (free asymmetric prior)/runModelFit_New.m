...
SetStartPoint = 0;
initialValueAll = [4    7        0.0000     30   -30     2    2.0902    0.5          0.5];
                %[stdNoise                  lapseRate  priorRange   stdMem  stdMotor smoothPrior    pCW]
binnedLH = 0;
plotFitProgress = 0;
plotEstFit = 0;
subjectID = {'ll', 'an', 'ep', 'jp'};
expNumber = 1;
...
for ll = 1 : length(subjectID)
    ...
    %% Extract the data
    [binaryDecision, percentCW, nTrialsPerCondition, estimateData, angleDiff, stdMotor] = dataForFittingNew(subjectID(ll), includeIncongruentTrials, correctTrialOnly);
        
    save('dataAll', 'angleDiff', 'estimateData',...
             'stdMotor', 'nTrialsPerCondition', 'percentCW')
    if fixMotorNoise
        initialValue(end-1) = mean(stdMotor);
    end
    ...
    counter = 1;
    for kk = 1 : nLoops
        [tempFitParameter, tempNegLLH] = modelFitBayes_New01(optimizationAlgorithm, SetStartPoint, initialValue, fileID, expNumber, plotFitProgress, modelType, '', fixMotorNoise, includeIncongruentTrials, fixLapseRate, fixSmoothPrior, fixPcw);
