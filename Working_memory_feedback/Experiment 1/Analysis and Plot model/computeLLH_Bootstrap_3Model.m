...
paramModel =        [2.7354    6.1605           0.0000     21.7403     1.5041   0.9972    2.0976;
...
tic
logLH_AllModel = NaN(length(subjID), 4, maxIter+1);
...
logLH_DataRep = repmat(logLH_Data, 1, 1, 4);
logLH_DataRep = permute(logLH_DataRep, [1 3 2]);
lowerBound = repmat(logLH_AllModel(:, 4, :), 1, 4, 1);
logLH_ModelNorm = (logLH_AllModel - lowerBound) ./ (logLH_DataRep - lowerBound);

meanLogLH = squeeze(logLH_ModelNorm(:, 1:3, end));
lowerConfLogLH = NaN(length(subjID), 3);
upperConfLogLH = NaN(length(subjID), 3);
